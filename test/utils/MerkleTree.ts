...
export function MiMC7(mimc7: MiMC, left: string, right: string): string {
    // Ensure the left and right strings are exactly 64 characters long and properly prefixed
    let left_hex = left.startsWith("0x") ? left : "0x" + left.padStart(64, '0');
    let right_hex = right.startsWith("0x") ? right : "0x" + right.padStart(64, '0');

    let hash = mimc7.multiHash([left_hex, right_hex]);
    let hex = mimc7.F.toString(hash, 16);
    let padded_hex = "0x" + hex.padStart(64, '0');
